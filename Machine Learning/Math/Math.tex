...
\item Maximum Likelihood Estimation for Gaussian
	...
	\item Analysis
		...
		as calculating $\sigma^2$ needs $\mu$, which help pin down the $x_N$ given $x_1,...,x_{N-1}$ \\
		$\Rightarrow $ unbiased variance $\displaystyle \hat \sigma^2 = \frac N {N-1}\sigma^2_\text{ML} = \frac N{N-1} \sum_{n=1}^N (x_n-\mu_\text{ML})^2 $
		\end{itemize}
	\end{itemize}
