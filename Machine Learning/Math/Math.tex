...
\subsubsection{Homogeneous Coordinate}
\begin{itemize}
\item Derivation
	\begin{itemize}
...
%		\item $\displaystyle \Rightarrow f(t_1,...,t_k)-f(a_1,...,a_k) = \sum_{i=1}^kD_if(a_1,...,a_k)(t_i-a_i) + E$ \\
%		$\displaystyle \phantom{\Rightarrow f(t_1,...,t_k)-f(a_1,...,a_k)} = \sum_{i=1}^k D_if(g_1(x_0), ...,g_k(x_0))(g_i(x)-g_i(x_0)) + E$ \\
%		$\displaystyle \Leftrightarrow \frac{f(g_1(x), ..., g_k(x))-f(g_1(x_0), ...,g_k(x_0))}{x-x_0} = \sum_{i=1}^k\frac{D_if(g_1(x_0), ..., g_k(x_0))}{x-x_0} + \frac{E}{x-x_0}$ \\
%		$\displaystyle \phantomsection{\Leftrightarrow \frac{f(g_1(x), ..., g_k(x))-f(g_1(x_0), ...,g_k(x_0))}{x-x_0}} =\sum_{i=1}^k\left[ \frac{D_if(g_1(x_0), ..., g_k(x_0))}{g_i(x)-g_i(x_0)}\frac{g_i(x)-g_i(x_0)}{x-x_0} \right] + \frac{E}{x-x_0}$
...
\item Multivariate - with Multiple Independent Variables
	...
	\item Statement
%		\begin{itemize}
%		\item with function $f:\mathbf R^m\rightarrow\mathbf R^k, g:\mathbf R^n\rightarrow\mathbf R^m$, a point $\mathbf a\in \mathbf R^n$ \\
%		$\Rightarrow$ 
%		\end{itemize}
	\end{itemize}
\end{itemize}
...
\subsubsection{Gaussian Integral} \label{Math_Calc_GaussianInt}
\begin{itemize}
...
\item Error Function
	...
	\end{itemize}
\end{itemize}
\subsubsection{Calculus of Variations} \label{Math_Calc_VarCalc}
\begin{itemize}
...
\item Basic Rules
	...
	\item Derivation
		...
		\includegraphics[width=\linewidth,center]{./Math/"prob-derivation of sum-product rule".png}
		\end{minipage}
...
\subsubsection{Bayesian Interpretation of Probability}
\begin{itemize}
\item Bayes' Rule
	\begin{itemize}
	\item Prior
...
\item Univariate Gaussian
	...
	\item Expectation
		...
		&= \sigma^2 + \mu^2, \text{ the 2nd moment of Guassian, by } (x {\rm e}^{-x^2})' = e^{-x^2}-2x^2{\rm e}^{-x^2}
		\end{align*}
		note: see \hyperref[Math_Calc_GaussianInt]{Gaussian integral} for $\int_R {\rm e}^{-x^2}\D x$
		\end{itemize}
	\item Variance
		\begin{itemize}
...
\item Multivariate ($d$-dimensional) Gaussian
	...
	\item Probability Dense Function (PDF)
		\begin{itemize}
		\item $\displaystyle \mathcal N_d(\mathbf x|\mathbf{\mu},\mathbf \Sigma) = \frac 1 {(2\pi)^{d/2} |\mathbf \Sigma|^{1/2}} \exp \left\{ -\frac 1 2 (\mathbf \mathbf x-\mathbf \mu)^T \mathbf \Sigma^{-1} (\mathbf x-\mathbf \mu) \right\} $, \\
		noted as $X\sim \mathcal N_d (x|\mu,\Sigma)$
		\end{itemize}
	\end{itemize}
\end{itemize}

\subsubsection{Operation on Gaussian}
\begin{itemize}
