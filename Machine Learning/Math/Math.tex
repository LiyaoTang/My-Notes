...
\item Introduction
	...
	\item Definition
		...
		note: map $\psi$ called exterior multiplication, element $\psi(\mathbf v_1\wedge...\wedge\mathbf v_k)$ called $k$-blade
		\end{itemize}
	\item Key Properties for Pair $(\wedge^kV,\psi)$
		\begin{itemize}
		...
		\item dim $\wedge^kV=\left(\begin{smallmatrix} n \\ k \end{smallmatrix}\right) = C^k_n$ (due to the form of its basis), \\
				where $n=$ dim $V$
		\item sum of $k$-wedge is still in the vector space $\wedge^k V$
		\item any alternating multilinear map $F:\underbrace{V\times...\times V}_k\rightarrow X$ factors uniquely into: \\ 
		$\underbrace{V\times...\times V}_k\xrightarrow{\psi} \wedge^k V \xrightarrow{\text{ }\underline F\text{ }} X$, where $\psi$
		exterior multiplication, $\underline F$ a linear map
		\end{itemize}
	\end{itemize}
\end{itemize}
