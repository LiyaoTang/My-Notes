...
\item Maximum Likelihood
	...
	\item Solution
		\begin{itemize}
		\item likelihood $\displaystyle p(\mathbf t| X,\mathbf w,\beta) = \prod_{n=1}^N \mathcal N(t_n|\mathbf w ^T\phi(\mathbf x_n), \beta^{-1})$ \\ (conditioning on $X$ as noise are introduced after input is given)
		\item $\Rightarrow \displaystyle \ln p(\mathbf t|X,\mathbf w,\beta) = - \frac \beta 2\sum_{n=1}^N (y(x_n, \mathbf w) - t_n)^2 + \frac N 2\ln\beta - \frac N 2 \ln(2\pi)$
			\begin{itemize}
...
\item Procedure:
	...
	but number on the diagonal line of $\lambda$ can be small \\
	$\Rightarrow$ maybe 0 depending on accuracy of computer
	\end{itemize}
\end{itemize}
