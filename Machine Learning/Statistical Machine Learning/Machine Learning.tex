...
\item $\displaystyle J(\theta)= \space... + \frac{\lambda}{2m}\sum^n_{j=1}\theta_j^2$ , \textbf{$\theta_0$ shouldn't be penalized} 
\item Matlab/Octave tips:

	\begin{itemize}
	\item Unrolling parameters: deltaVector = [ D1(:) ; D2(:) ] // matrix D1,D2 => column vector 
	\item Reshape: D1 = reshape (deltaVector(1, m*n), m, n) // column vector $\Rightarrow$ m*n matrix
	\end{itemize}
...
\item Assumption: 
	\begin{itemize}
	\item \(y=h^\star_\theta(x)+\epsilon \text{, where $h^\star_\theta(x)$ is the theoretical (perfect) hypothesis, $\epsilon$ is gaussian noise}\)
	\end{itemize}

\item General Hypothesis Function: $h_\theta(x)=\phi (x)\theta$, where $\phi (x)$ is basis function

\item Log Likelihood leads to least square cost function $J(\theta)$ 

	\begin{itemize}
	\item $P(y|x,\theta, \beta) = \mathcal{N}(x|h_\theta(x), \beta^{-1})$ - likelihood
	
		\begin{itemize}
		\item $\text{based on assumption: } y=h^\star_\theta(x)+\epsilon, \text{where}$ \\
		\( h^\star_\theta(x) \text{ is the theoretical (perfect) hypothesis, $\epsilon$ is gaussian noise}\)
		\end{itemize}
	\item $\displaystyle P(\boldsymbol y|X,\theta,\beta) = \prod_{i=1}^m \mathcal{N}(x^i|h_\theta(x^i), \beta^{-1})$ 
