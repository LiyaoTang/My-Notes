...
\item Fast R-CNN
	...
	\item Inference
		...
		\item one-pass inference: shared computation for feature from backbone CNN \\
		(similar to SPP \& improved over R-CNN)
	...
	\item Training
		...
		($0.1$ to ensure hard negative \& pos example $< 25\%$)
	...
	\item Understanding
		...
		(by exploring a more robust shared representation)
		\item faster training: sharing computation in batch \& no SVM
		\item faster inference: network compression by SVD on weight matrix of dense layer
		...
		\item update on loc \& cls branch are decoupled (not considering pred class) \\
		$\Rightarrow$ \underline{MISalignment} with final evaluation (precision/recall are class-specific)
		\end{itemize}
	\end{itemize}
