...
\item Gated Association
	...
	\item Procedure
		\begin{itemize}
		\item given prediction, its variance and observation noise, filter out an interested area
		\item consider only observation inside the interested area (satisfying requirements)
		\item score each observation \& associate observation (detection result) with tracker
		$\Rightarrow$ which observation belongs to which trajectory
		\end{itemize}
	\item Global Nearest Neighbor
		\begin{itemize}
		\item choose the best / most probable / nearest
		\item under the constraint that an observation can associate with at most one track)
		\end{itemize}
	\item Nearest Neighbor
		\begin{itemize}
		\item choose the best / most probable / nearest \\ 
		\item thought one observation may be used by multiple track(s)
		\end{itemize}
		
	\end{itemize}
\item Joint Probability Data Association
	\begin{itemize}
	\item 
	\end{itemize}
\item Graph Matching with Affinity Constraint
	\begin{itemize}
	\item 
	\end{itemize}
