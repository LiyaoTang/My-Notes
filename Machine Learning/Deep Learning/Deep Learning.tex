...
\item You Only Look Once (YOLO)
	...
	\item Optimization Goal
		\begin{itemize}
		\Item \begin{align*} \displaystyle \text{loss} = & \lambda_\text{obj}\sum_{i=0}^{S^2}\sum_{j=0}^{B} \mathbb{1}^{obj}_{ij} [(x_i-\hat x_i)^2 + (y_i-\hat y_i)^2] +\\
		& \lambda_\text{obj}\sum_{i=0}^{S^2}\sum_{j=0}^{B}\mathbb 1^{obj}_{ij} [(\sqrt{w_i}-\sqrt{\hat w_i})^2 + (\sqrt{h_i}-\sqrt{\hat h_i})^2] + \\
		& \sum_{i=0}^{S^2}\sum_{j=0}^{B}\mathbb 1^{obj}_{ij} (conf_{ij} - \hat {conf}_{ij})^2 + \lambda_\text{none}\cdot\mathbb 1^{none}_{ij} (conf_{ij} - \hat {conf}_{ij})^2 +\\
		& \sum_{i=0}^{S^2}\mathbb 1^{obj}_{i}\sum_{c=0}^C (p_{ci} - \hat p_{ci})^2 \end{align*}
		\item $\lambda_\text{obj} = 5$ to up-weight box with obj, $\lambda_\text{none}=0.5$ for empty box
			...
			\end{itemize}
		\item $\mathbb{1}^{obj}_{ij}$ the indicator: $1$ if obj exists in $j$ box of $i$ cell, similar for $\mathbb 1^{none}_{ij}$
...
\item Faster One-stage
	...
	\item Segmentation as Detection
		...
		\end{itemize}
	\end{itemize}
\item No Non-Maximum Suppression
	\begin{itemize}
	\item Auto Filtering
		\begin{itemize}
	...
	\item RNN Encoder-Decoder
		\begin{itemize}
		\item encode all bbox, then decode (generate) till end-of-sequence
		\end{itemize}
	\end{itemize}
\end{itemize}
...
\item Input Data
	...
	\item Raw Data
		\begin{itemize}
		\item directly from sensor $\Rightarrow$ need to embed detection
		\end{itemize}
	\item Object Representation
		\begin{itemize}
		\item bounding box: 2/3-D size, with pose, motion, velocity, etc. estimation available
...
\item Goal
	...
	\item Trajectory
		\begin{itemize}
		\item recover the true trajectory of the object, including current localization
		\end{itemize}
	\end{itemize}
...
\item Inversed Reinforced Learning for Data Association with Markov Decision Process
	...
	\item Output
		...
		\end{itemize}
	\item Markov Decision Process (MDP) for Association
		\begin{itemize}
		\item states: active, tracked, lost, inactive \\
		$\Rightarrow$ model the state of a track
		\item probability at each state given by trained model
		\end{itemize}
	\item Inversed Reinforcement Learning for Lost Recovery
		\begin{itemize}
		\item regression model to associate lost track \& current detection
		\item regressor trained only when MDP make wrong decision \\
...
\item Siamese Network
	...
	\item Input
		\begin{itemize}
		\item detection pair: one from current frame, another from last frame
		\end{itemize}
	\item Output
		\begin{itemize}
		\item confidence that detection pair belongs to the same track
		\end{itemize}
	\end{itemize}
\end{itemize}

\subsubsection{Online-trained Tracker}
\begin{itemize}
\item Overview
	\begin{itemize}
	\item Understanding
...
\subsubsection{Offline-trained Tracker}
\begin{itemize}
\item Overview
	\begin{itemize}
	\item Understanding
...
\item Overview
	...
	\item Understanding
		\begin{itemize}
		\item offline trained detector, online updating tracker
		\item yet, speed limited by tracker (as online update)
		\end{itemize}
	\end{itemize}
...
\item Re$^3$: Real-time Recurrent Regression Network
	...
	\item Tracking Initialization
		\begin{itemize}
		\item image sequence with an initial bounding box over arbitrary object
		\end{itemize}
	\item Detector
		\begin{itemize}
		\item CNN to extracts multi-scale representation from image \\
		$\Rightarrow$ more descriptive info (human in red/blue shirt)
		\end{itemize}
	\item Tracker
		\begin{itemize}
	...
	\item Inference
		...
		\item tracker LSTM updates on fused info
		\item dense layer regress the tracker output at last layer for bounding box at $t+1$
		\end{itemize}
	\item Training
		\begin{itemize}
...
\item End-to-end tracking
	...
	\item Track Initiation/Deletion
		...
		\end{itemize}
	\item Interesting Predicted Bounding Box Encoding
		\begin{itemize}
		\item direct encoding: x,y,w,h + full image
		\item mask: bounding box plot onto a separate mask, concat to the full image \\
		\item further, attention mechanism
		\end{itemize}
		$\Rightarrow$ enable global track for multiple obj track (single RNN for all track in an image)
	\item End-to-End MOT
		\begin{itemize}
		\item image $t$ encoded by cnn: $f_t$
		\item regress the bbox(es) $d_t$; encode the bbox(es) by rnn encoder $c(d_t)$
		\item fed $f_t, c(d_t)$ to rnn tracker, and output $o_t$ \\
		(track history in tracker rnn state)
		\item decode $o_t$ by another rnn decoder, to predict bbox(es) \\
		(generate till end-of-sequence)
		\end{itemize}
	\end{itemize}
\end{itemize}
