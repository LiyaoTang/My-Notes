...
\item Evaluation Metrics
	...
	\item Success Plot
		\begin{itemize}
		\item success: IoU of pred \& label larger than a threshold
		\item $\Rightarrow$ setting a series of threshold from $0$ to $1$ to obtain a line
		\end{itemize}
	\item VOT Metrics
		\begin{itemize}
...
\item SiamRPN: Tracking with Siamese Region Proposal Network
	\begin{itemize}
	...
	\item Tracking
		\begin{itemize}
		\item with prepared $\varphi(z) \& \zeta[\varphi(z)]$, template branch pruned for speed up
	...
	\item Training
		...
		$\Rightarrow$ treat box as square \& has always the same percentage of target in the input
	...
	\item Understanding
		...
		$\Rightarrow$ NOT able to handle large search region (use only limited center size in refinement) \\
		(not discriminative enough)
		\item yet, using both VID \& youtub-bb dataset \\
		$\Rightarrow$ youtube-bb contain larger transform between frames (due to one labeled frame in every 30 frames) \\
		$\Rightarrow$ needs larger center-size \& context to overcome large transformation
		\item tracking based on both \underline{similarity v.s. transformation v.s. semantic } \\
		...
		\item semantic: from using RPN \& pretrain on classification dataset \& tracking only general object \\
		$\Rightarrow$ use anchor for obejct different $w$-$h$ ratio
...
\item C-RPN: Siamese Cascaded Region Proposal Network
	...
	\item Tracking Initialization
		\begin{itemize}
		\item extract feature of template branch $z$ based on bbox at $1^\text{st}$ frame
		\end{itemize}
	\item Detector
		\begin{itemize}
	...
	\item Inference
		...
		\item $l^\text{st}$ RPN convolute $\Phi_{l}(z)$ on $\Phi_{l}(x)$ \\
