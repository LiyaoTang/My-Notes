...
\item Gated Association
	...
	\item Procedure
		\begin{itemize}
		\item skipped if no explicit detection performed
		\item single association: associate observation (detection result) with tracker \\
		$\Rightarrow$ which observation belongs to which trajectory
		\item global association: each trajectory considers all observation (with weighting)
		\end{itemize}
	\end{itemize}
