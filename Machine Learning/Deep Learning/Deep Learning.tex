...
\item End-to-end / unified framework
	...
	\item Track Initiation/Deletion
		...
		\end{itemize}
	\item \underline{Interesting Predicted Bounding Box Encoding}
		\begin{itemize}
...
\item Spatial Pyramid Pooling (SPP)
	...
	\item Operation
		...
		\includegraphics[width=0.5\linewidth, center]{"./Deep Learning/layer-pooling spp".png}
		\end{itemize}
	\item Output
		\begin{itemize}
...
\item Basic Image-forming Theory
	...
	\item Focal Length $F$
		...
		\includegraphics[width=0.5\linewidth, left]{"./Deep Learning/topic-vision sensor  camera focal len".jpg}
		\end{itemize}
	\item Aperture
		\begin{itemize}
	...
	\item Depth of Field (DOF) \& Depth of Focus
		...
		\includegraphics[width=0.5\linewidth, left]{"./Deep Learning/topic-vision sensor  camera dof".jpg}		
		\end{itemize}
	\item Depth of Field
		\begin{itemize}
		\item DOF as the range where $1$ pixel can hold all info from $1$ position \\
		\includegraphics[width=0.7\linewidth, left]{"./Deep Learning/topic-vision sensor  camera n and dof".png} \\
		...
		\includegraphics[width=0.7\linewidth, left]{"./Deep Learning/topic-vision sensor  camera dof and resolution".png} \\
	...
	\item Depth of Focus
		...
		\includegraphics[width=0.7\linewidth, left]{"./Deep Learning/topic-vision sensor  camera do focus".png} \\
...
\item Camera System
	...
	\item Color Filter Array
		...
		\includegraphics[width=0.7\linewidth, left]{"./Deep Learning/topic-vision sensor camera color filter array".png}
		\end{itemize} 
	\item ISP
		\begin{itemize}
...
\item YOLOv3
	...
	\item Structure
		...
		\begin{figure}[ht]
		\includegraphics[width=0.9\linewidth, right]{"./Deep Learning/topic-objdet onestage yolov3 architecture".png}
		\end{figure}
		\end{itemize}
	\item Optimization
		\begin{itemize}
...
\item Overview
	...
	\item Online Tracking
		...
		\end{itemize}
	\item Near-Online Tracking
		\begin{itemize}
		\item look ahead a fixed number of frames, before consolidating a method
		\end{itemize}
	\item Offline Tracking
		\begin{itemize}
		\item learn similarity functions between frames offline
		\item fast, as no online training needed
		\item NOT adaptive $\Rightarrow$ failed in large appearance change, e.g. occlusion etc.
...
\item Evaluation Metrics
	\begin{itemize}
	\item Percentage of Time Step with Success
		\begin{itemize}
		\item overlap success: IoU of pred-label bounding box $\ge$ threshold $=0.25$
...
\item Interacting Multiple Model
	\begin{itemize}
	\item 
	\end{itemize}
...
\item GOTURN: 
	...
	\item Training
		\begin{itemize}
		\item train with consecutive image pair from sequences
		\item $L1$ loss for exact match
		\item data augment: motion noise applied on the $2^\text{th}$ image; random crop on the $1^\text{st}$\\ 
		(distribution of motion noise chosen from cross-validation)
		\end{itemize}
	\item Understanding
		\begin{itemize}
		\item $2$-frame model, $1$-step prediction, no explicit temporal tracking \\
		$\Rightarrow$ net may learn to locate the nearest similar object
		\item real-time timing ($100$fps), included in opencv $\Rightarrow$ stable performance
		\end{itemize}
	\end{itemize}

\item CFNet: Correlation Filter based Tracking
	\begin{itemize}
	\item Inference
	\item Structure
	\item Training
...
\item Re$^3$: Real-time Recurrent Regression Network
	...
	\item Understanding
		\begin{itemize}
		\item end-to-end training for both detector and tracker (improved GOTURN)
		\item model regress changes to the box ratio $\Rightarrow$ easier
		\item observation usefulness modeled by LSTM tracker
		...
		\item LSTM tracker maintain track state \\ 
...
\item C-RPN: Siamese Cascaded Region Proposal Network
	...
	\item Tracking Initialization
		\begin{itemize}
		\item bounding box at first frame as ground truth / feature template
		\end{itemize}
	\item Detector
		\begin{itemize}
		\item fully conv siamese net for encoding (multi-scale info utilized) \\
		$\Rightarrow$ extracts info from target template $x$ and search region $z$ (gating) \\
		...
		\item $f(\Phi_{l-1}, \phi_l) = $feature transform block \\
	...
	\item Inference
		\begin{itemize}
		\item siamese net extracts features from target template \& search region
		\item $l^\text{st}$ RPN takes input $\Phi_{l}(x), \Phi_{l}(z)$, regresses based on anchor boxes $A_{l}$
		\item discard any boxes $\in A_l$ with confidence/objectness lower than a threshold \\
		$\Rightarrow$ produce anchor boxes $A_{l+1}$
		\item fuse semantic info with lower-level info, for both branch for $x, z$
		\item $t^\text{th}$ RPN takes 
		\end{itemize}
	\item Loss
		\begin{itemize}
	...
	\item Tracking
		\begin{itemize}
		\item arbitrary target object selected for tracking (treated as ground truth) \\
		$\Rightarrow$ extract the feature template
		\item gating on frame $t$ based on $t-1$ result
		\item C-RPN detect target within gated region of frame $t$ \\
...
\item Fast Online Object Tracking and Segmentation
	\begin{itemize}
	\item Detector
		\begin{itemize}
		\item 
...
\item PCB-RPP: Person Retrieval with Refined Part Pooling
	...
	\item Understanding
		...
		\begin{minipage}[r]{0.5\linewidth}
		\includegraphics[width=\linewidth, left]{"./Deep Learning/topic-reid pcb-rpp within-part inconsistency".png}
		\end{minipage}
		\begin{minipage}[l]{\linewidth}
		where, \\
...
\item MGNet: Discriminative Features with Multiple Granularities
	...
	\item Inference
		...
		\item all representation concat as global representation
		\end{itemize}
	\item Structure
		\begin{itemize}
		\item ResNet-50 as backbone
		\item global branch:
		\item part $1$ branch: $1$ vertical split ($2$ strips)
		\item part $2$ branch: $2$ vertical split ($3$ strips)
		\end{itemize}
	\item Training
		\begin{itemize}
		\item 
	...
	\item Understanding
		\begin{itemize}
		\item multi-scale feature $\Rightarrow$ global + local at different scale
		\item provide only equal split of image: \\
		$\Rightarrow$ let network itself determine the feature of each split
		\item less \& simpler prior: only fixed split without explicit structure model
		\end{itemize}
	\end{itemize}
\end{itemize}
...
\subsection{Temporal Generative Adversarial Nets}
