...
\item Input Data
	...
	\item Raw Data
		\begin{itemize}
		\item need to embed detection
		\end{itemize}
	\item Detection Result
		\begin{itemize}
		\item bounding box: 2/3-D size, with pose, motion, velocity, etc. estimation available
		\item point model: no direct size, centroid point with attributes denoting an object
		\end{itemize}
	\end{itemize}
\item Goal
	\begin{itemize}
...
\begin{itemize}
\item Point-Kalman Filter
	\begin{itemize}
	\item Detector
		\begin{itemize}
		\item basic rule/statistical model
		\item point model as detection result
		\end{itemize}
	\item Gated Association
		\begin{itemize}
		\item search for detection in a vicinity of prediction; if failed, prediction used \\
		$\Rightarrow$ an approximation of JPDA (joint probability Data Association)
		\end{itemize}
	\item Tracker
		\begin{itemize}
		\item classic KF, for location, velocity, accelerate
		\end{itemize}
	\item Orientation
		\begin{itemize}
		\item frame differencing of current-previous location
		\end{itemize}
	\end{itemize}
\end{itemize}
