...
\item Region of Interest Pooling (RoI Pooling) \label{DL_Layers_Pooling_roi}
	...
	\item Input
		\begin{itemize}
		\item feature maps from CNN
		\item RoIs i.e. proposal region (from selective search, RPN etc.) projected on feature map
		\end{itemize}
	\item Operation
		\begin{itemize}
...
\item Selective Search
	...
	\item Understand
		...
		\end{itemize}
	\end{itemize}
\item Anchor Scale Normalization
	\begin{itemize}
	\item Input
		\begin{itemize}
		\item 
...
\item RPN: Faster R-CNN
	...
	\item Inference
		...
		$\Rightarrow$ channel-size = $2k$ for cls; $4k$ for reg
	...
	\item Understanding
		...
		(though not entirely end-to-end due to NO non-approximate joint training)
			\begin{itemize}
			...
			\end{itemize}
		\item \underline{anchors} for multi-scale detection $\Rightarrow$ detection reference pyramid \\
...
\item Feature Pyramid Network (FPN)
	...
	\item Understanding
		...
		\item prediction independently made at each scale, by a shared pred head \\
		(analogs to using image pyramid \& non-share has similar performance) \\
		$\Rightarrow$ easily generate to pyramid with more/less levels
