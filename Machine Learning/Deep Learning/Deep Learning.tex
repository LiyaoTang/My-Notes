...
\item SiamRPN: Tracking with Siamese Region Proposal Network
	...
	\item Understanding
		\begin{itemize}
		\item $2$-frame model: tracking as one-shot detection \\
		...
		\item proposal refinement by RPN to replace online fine-tuning
		...
		\item detection branch able to propose \& refine a compact bbox \\ 
		$\Rightarrow$ no need for online learning, nor multi-scale test
		\item NOT able to handle large search region (use only limited center size in refinement) \\
