...
\item You Only Look Once (YOLO)
	...
	\item Preprocessing
		...
		\item assign the objects to a tuple (cell, anchor box)
			\begin{itemize}
			\item assign to cell by its central point $\Rightarrow$ try ensuring maximal $\mathbf 1$ object in a grid
	...
	\item Inference
		...
		i.e. the class probability of object, given the object assigned to the cell
	...
	\item Limitation
		...
		\item NOT able to handle: multiple objects in same-type anchor box in one cell \\
...
\item YOLOv2 (YOLO9000)
	...
	\item Anchor Box Selection
		\begin{itemize}
		\item run K-means on all label box; use cluster center as the anchor box
		\end{itemize}
	\item Bounding Box Encoding
		\begin{itemize}
		\item $p_e$ confidence of current box containing an object \\
		()
	...
	\item Prediction
		\begin{itemize}
		\item each cell predict $B$ bounding boxes with their $p_e$
