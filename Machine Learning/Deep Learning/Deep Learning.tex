...
\item Encoder-Decoder Attention
	...
	\item Motivation
		\begin{itemize}
		\item overcome long-term dependency, via considering all input at each step
...
\item Self Attention
	...
	\item Operation
		...
		$\Rightarrow$ generate query $q=W^Q\cdot e$, key $k=W^K\cdot e$ and value $v=W^V\cdot e$, where \\
		...
		\item normalize by the square root of the length of the "key" vector \\
	...
	\item Speed-up
		...
		\item $\Rightarrow A = \text{softmax}\left( \frac 1 {\sqrt{d_k}} Q^T\cdot K \right)$, \\
		where $A_i$ the attention for $i^\text{th}$ word (as a row vector)
		\item $\Rightarrow C = V\cdot A^T$, where each context vector as a col vector
		\end{itemize}
	\item Understanding
		\begin{itemize}
...
\item Multi-headed Self Attention
	...
	\item Operation
		\begin{itemize}
		\item the "batch-mode" of the self attention to get $N$ context matrices
...
\item Multi-Camera Vehicle Tracking and Re-Identification
	...
	\item Multi-Camera Tracking
		...
		$S_d$ the similarity of vehicle motion direction (in a uniformed coord), \\
		$S_t$ the estimated-vs-true traveling time similarity (if cam not overlaps)
...
\item Transformer: Attention is All You Need
	...
	\item Embedding
		\begin{itemize}
		\item word embedding + positional embedding
		\end{itemize}
	\item Structure
		\begin{itemize}
		...
		\item multi-headed self attention
		\item encoder-decoder attention ???
		\item output by softmax classifer as distribution over all corpus
		\end{itemize}
	\item Training
		\begin{itemize}
		\item
	...
	\item Understanding
		\begin{itemize}
		\item no sequential component use in encoding $\Rightarrow$ parallelized encoding phase
		\end{itemize}
	\end{itemize}
