...
\item Overview
	...
	\item Understanding
		...
		\item fast, as no training needed
...
\item Re$^3$: Real-time Recurrent Regression Network
	...
	\item Inference
		\begin{itemize}
		\item crop $t^\text{th}$ image: center at current bounding box, extended to twice of box size
		\item crop $t+1^\text{th}$ image: center at bounding box of $t$, twice size
...
\item End-to-end tracking
	...
	\item End-to-End Regression for Data Association in MOT
		...
		\item concern for multiple objects: multiple pred, det, track
			\begin{itemize}
			\item track oriented: each track+pred, consider all det $\Rightarrow$ ranking the det \\
			(basically, the current literature?)
			\item track+all det: concat (tile) track+pred to each det $\Rightarrow$ use fcn, then pooling to regress to a bbox for update \\
			$\Rightarrow$ fully NN-approach for JPDA
			\end{itemize}
		\item directly output decision result of track initiation/deletion \\
	...
	\item End-to-End MOT
		...
		\item decode $o_t$ by another RNN decoder (for prediction)
		\end{itemize}
	\end{itemize}
\end{itemize}
