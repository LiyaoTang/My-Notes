...
\item Transposed Convolution
	...
	\item Setting
		\begin{itemize}
		\item square $i\times i$ input \& $k\times k$ kernel
		\item same stride $s$ \& (one-direction) padding $p$ for all axises
		\end{itemize}
	\item Operation
		\begin{itemize}
		\item given $i\times i$ input, $k\times k$ kernel, stride $s$ \& padding $p$
		\item construct matrix $C$ for forward, pad input and then unroll into $\mathbf x$
		\item 
		\end{itemize}
	\item Output
		\begin{itemize}
