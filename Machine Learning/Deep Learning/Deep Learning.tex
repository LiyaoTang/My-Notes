...
\item State-of-the-Art
	...
	\item Deep Power
		\begin{itemize}
		\item revealed to be central bias in deepnet
		\end{itemize}
...
\item SiamRPN++
	...
	\item Analysis
		...
		\item YET, deep net involves too much padding \\
		(feature most densely aggregated in the center of feature map) \\
		$\Rightarrow$ translation in input $\mathbf z, \mathbf x$ NOT reflected in embedded space $\phi(\mathbf z), \phi(\mathbf x)$ \\
		($0$-padding creates higher response in the central \& label target mostly sit in the center) \\
		$\Rightarrow$ too much padding prevents xcorr from effectively measuring simiarity
		$\Rightarrow$ destroy the translation invariance \& introduce \underline{central bias}
		\item if training with no shift augmentation \\ 
		(random shifting the crop region s.t. target not right in the center) \\
		$\Rightarrow$ high-prop prediction on test set centralize on the center \\ 
		(despite the between-frame movement of target) \\
		whereas, trained with shift augmentation $\Rightarrow$ no central bias in predicted prob
		\end{itemize}
	\item Understanding
		\begin{itemize}
		...
		\item deep net can learn spatial bias (in siam tracker case: central bias), due to padding \\
		$\Rightarrow$ shift augmentation to force network to be location invariance \\
		$\Rightarrow$ force network to learn effective appearance model to measure similarity
